...
import { ConsumoExtraDialog } from "../checkin/consumo-extra-dialog"
import { formatCurrency, formatDate, formatDateTime } from "@/lib/utils"
import { EstadoHospedaje } from "@prisma/client"
import { User, Bed, Calendar, Receipt, ShoppingCart, Clock, MapPin, CreditCard } from "lucide-react"
...
interface HospedajeDetalleProps {
  hospedaje: HospedajeWithDetails
  productos: Producto[]
}

export function HospedajeDetalle({ hospedaje, productos }: HospedajeDetalleProps) {
  ...
  const totalPagado = hospedaje.factura?.pagos.reduce((sum, pago) => sum + pago.monto, 0) || 0
  const saldoPendiente = (hospedaje.factura?.total || 0) - totalPagado
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center space-x-2">
                  ...
                  <span>Período de Hospedaje</span>
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-semibold">{diasHospedaje} días</p>
                  </div>
                  {hospedaje.estado === EstadoHospedaje.ACTIVO && (
                    <div>
                      <label className="text-sm font-medium text-gray-500">Días Transcurridos</label>
                      <p className="text-lg">{Math.max(0, diasTranscurridos)} días</p>
                    </div>
                  )}
                </div>
              </CardContent>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{formatCurrency(hospedaje.factura.total)}</span>
                      </div>
                      <div className="flex justify-between">
          ...
          <CardContent className="space-y-4">
            ...
            {hospedaje.estado === EstadoHospedaje.ACTIVO && (
              <div className="flex justify-between items-center">
                ...
                <span>{Math.max(0, diasTranscurridos)} días</span>
              </div>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-3">
            ...
            {hospedaje.factura ? (
              <>
                <div className="flex justify-between text-lg font-bold">
                  <span>Total Facturado</span>
                  <span>{formatCurrency(hospedaje.factura.total)}</span>
                </div>
